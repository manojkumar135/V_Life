...
Font.register({
  ...
  fonts: [
    ...
    {
      src: `${BASE_URL}/fonts/Roboto/Roboto-Bold.ttf`,
      fontWeight: "bold",
    },
  ],
});
  ...
  return (
    <Document>
      {invoices.map((invoice, index) => (
        <Page key={index} size="A4" style={[styles.page, { marginLeft: 1, marginRight: 1 }]}>
          {/* ---------- HEADER ---------- */}
          ...
          <View style={{ marginLeft: 3 }}>
            ...
            <Text>
              ...
              {invoice.office?.office_landmark ? `, ${invoice.office.office_locality}` : ""}
              {invoice.office?.office_city ? `, ${invoice.office.office_state}` : ""}
              {invoice.office?.office_country ? ` - ${invoice.office.office_pincode}` : ""}
            </Text>
          ...
          <View style={styles.addressContainer}>
            ...
            <View style={styles.colLeft}>
              ...
              <View style={{ marginLeft: 5 }}>
                ...
                <Text style={{ margin: 5, marginLeft: 10, textTransform: "capitalize", fontSize: 9, marginBottom: 30 }}>{invoice.order?.address || "N/A"}</Text>
              </View>
          ...
          <View style={styles.table}>
            ...
            {invoice.order?.items?.map((product, idx) => {
              ...
              const taxableAmount = product.whole_gst || (dealerPrice * (product.gst_rate || 0) / 100) || 0;

              return (
                <View key={idx} style={styles.row}>
                  <Text style={[styles.col, styles.cNo, { textAlign: "right" }]}>{idx + 1}</Text>
                  <Text style={[styles.col, styles.cCode, { textAlign: "center" }]}>{product.product_code || ""}</Text>
                  <Text style={[styles.col, styles.cName, { textTransform: "capitalize" }]}>{product.name || ""}</Text>
                  <Text style={[styles.col, styles.cHSN, { textAlign: "center" }]}>{product.hsn_code || ""}</Text>
                  <Text style={[styles.col, styles.cQty, { textAlign: "center" }]}>{quantity}</Text>
                  <Text style={[styles.col, styles.cPrice, { textAlign: "right" }]}>{`\u20B9 ${dealerPrice.toFixed(2)}`}</Text>
                  <Text style={[styles.col, styles.cTaxable, { textAlign: "right" }]}>{`\u20B9 ${taxableAmount.toFixed(2)}`}</Text>
                  ...
                  <Text style={[styles.col, styles.cTotal, { textAlign: "right" }]}>{`\u20B9 ${total.toFixed(2)}`}</Text>
                </View>
              );
            })}
          </View>
          ...
          <View style={styles.totalsSection}>
            <View style={styles.totalsRow}>
              ...
              <Text>{`\u20B9 ${((invoice.order?.total_amount || 0) - (invoice.order?.total_gst || 0)).toFixed(2)}`}</Text>
            </View>
            ...
            {/* ---- ADVANCE DEDUCTED ---- */}
            {(invoice.order?.is_first_order && invoice.order?.advance_deducted > 0) && (
              <View style={styles.totalsRow}>
              ...
              <>
                ...
                {invoice.order?.reward_usage?.cashback?.used > 0 && (
                  <View style={styles.totalsRow}>
                    <Text>Cashback</Text>
                    <Text style={styles.deduction}>
                      {`- \u20B9 ${(invoice.order.reward_usage.cashback.used || 0).toFixed(2)}`}
                    </Text>
                  </View>
                )}

                {/* Fortnight */}
                {(invoice.order?.reward_usage?.daily?.used > 0 || invoice.order?.reward_usage?.fortnight?.used > 0) && (
                  <View style={styles.totalsRow}>
            ...
            <View style={styles.totalsRow}>
              ...
              <Text style={{ fontWeight: "bold", fontSize: 10 }}>{`\u20B9 ${(invoice.order?.final_amount || 0).toFixed(2)}`}</Text>
            </View>
          </View>
