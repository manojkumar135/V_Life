...
interface WithdrawRow {
  ...
  neft_utr:             string | null;
  neft_transaction_date: string | null;
  // inline edit state — only used on the frontend
  _editing:             boolean;
  _utr:                 string;
  _date:                string;
  _time:                string;
...
export default function BatchDetailPage() {
  ...
  // ── Batch-level UTR form ──
  const [batchMode, setBatchMode]     = useState(false); // show/hide batch UTR form
  ...
  const fetchDetail = useCallback(async () => {
    ...
    try {
      ...
      // Pre-fill batch UTR form if already set
      ...
      // Build rows with inline edit state
  ...
  /* ── Batch UTR update (one UTR for ALL records) ── */
  ...
  /* ── Inline row edit helpers (selective mode) ── */
  const startEdit = (payoutId: string) => {
    setRows((prev) =>
      prev.map((r) =>
        r.payout_id === payoutId ? { ...r, _editing: true } : r,
      ),
    );
  };

  const cancelEdit = (payoutId: string) => {
    setRows((prev) =>
      prev.map((r) =>
        r.payout_id === payoutId
          ? { ...r, _editing: false, _utr: r.neft_utr || "", _date: r.neft_transaction_date || "", _time: "" }
          : r,
      ),
    );
  };

  const updateField = (payoutId: string, field: "_utr" | "_date" | "_time", value: string) => {
    setRows((prev) =>
      prev.map((r) =>
        r.payout_id === payoutId ? { ...r, [field]: value } : r,
      ),
    );
  };

  /* ── Save single row UTR (selective mode) ── */
  const saveRowUTR = async (row: WithdrawRow) => {
    ...
    setRows((prev) =>
      prev.map((r) => (r.payout_id === row.payout_id ? { ...r, _saving: true } : r)),
    );
    try {
      const { data } = await axios.patch(`/api/payrelease/batches/${batchId}`, {
        mode: "selective",
      ...
      if (data.success) {
        ...
        fetchDetail(); // refresh to get updated status + summary
      } else {
        ShowToast.error(data.message || "Save failed");
        setRows((prev) =>
          prev.map((r) => (r.payout_id === row.payout_id ? { ...r, _saving: false } : r)),
        );
      }
    } catch (err) {
  ...
  return (
    <Layout>
      <div className="p-4 max-md:p-2">
        ...
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#0C3978] text-white
                text-xs font-semibold hover:bg-[#106187] transition-colors cursor-pointer disabled:opacity-50"
            >
              <FiDownload size={14} />
        ...
        {batch && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            {[
              {
                label: "Released Date",
                value: `${batch.released_date} ${batch.released_time}`,
                color: "from-[#0C3978] to-[#106187]",
              },
              {
                label: "Users / Payouts",
                value: `${batch.user_count} users · ${batch.payout_count} payouts`,
                color: "from-[#106187] to-[#16B8E4]",
              },
              {
                label: "UTR Progress",
                value: `${summary.updated} / ${summary.total} done`,
                sub: `${summary.pending} pending`,
                color: summary.pending === 0 ? "from-green-600 to-green-500" : "from-orange-500 to-orange-400",
              },
              {
                label: "Total Released",
                value: `₹ ${Number(batch.total_amount).toFixed(2)}`,
          ...
          <div className="mb-4 rounded-xl border border-green-200 bg-green-50 px-5 py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              ...
              <InputField
                ...
                onChange={(e: any) =>
                  setBatchForm((f) => ({ ...f, neft_transaction_date: e.target.value }))
                }
              ...
              <InputField
                ...
                onChange={(e: any) =>
                  setBatchForm((f) => ({ ...f, neft_transaction_time: e.target.value }))
                }
                placeholder="HH:MM"
              />
              <InputField
            ...
            <div className="flex gap-2 mt-4">
              ...
              <button
                ...
                className="px-5 py-2 rounded-lg bg-gray-200 text-gray-700 text-sm font-semibold
        ...
        {/* ── Withdraw records table (selective per-row UTR edit) ── */}
        <div className="bg-white rounded-xl shadow overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
            ...
            <p className="text-xs text-gray-400">
              Click the edit icon on any row to set its UTR individually
            </p>
          </div>
              ...
              <tbody>
                ...
                {rows.map((row, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium
                          ${BONUS_COLORS[row.bonus_type] || "bg-gray-100 text-gray-600"}`}
                      >
                        {row.bonus_type}
                      </span>
                    </td>
                    ...
                    {/* UTR — show input if editing, else display value */}
                    <td className="px-3 py-2">
                      {row._editing ? (
                        <input
                          type="text"
                          ...
                          className="w-36 text-xs border border-blue-300 rounded px-2 py-1
                    ...
                    {/* NEFT Date — show input if editing */}
                    <td className="px-3 py-2">
                      {row._editing ? (
                        <div className="flex flex-col gap-1">
                          <input
                            ...
                            placeholder="DD-MM-YYYY"
                            className="w-28 text-xs border border-blue-300 rounded px-2 py-1
                              focus:outline-none focus:ring-1 focus:ring-blue-400"
                          />
                          <input
                            type="text"
                            ...
                            placeholder="HH:MM"
                            className="w-20 text-xs border border-blue-300 rounded px-2 py-1
                              focus:outline-none focus:ring-1 focus:ring-blue-400"
                          />
                        </div>
                      ) : (
                    ...
                    <td className="px-3 py-2 text-center">
                      {row._editing ? (
                        ...
                        <button
                          ...
                          title="Edit UTR"
                          className="p-1 rounded text-[#0C3978] hover:bg-blue-50
                            transition-colors cursor-pointer"
                        >
                          <FiEdit2 size={14} />
                        </button>
