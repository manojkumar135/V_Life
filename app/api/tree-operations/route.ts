...
function calculateInfinityValues(userMap: Map<string, any>) {
  for (const [id, user] of userMap) {
    ...
    user.leftBV = leftDocs.reduce(
      (s: number, u: { bv?: number } | undefined) => s + (u?.bv || 0),
      0,
    );
    user.rightBV = rightDocs.reduce(
      (s: number, u: { bv?: number } | undefined) => s + (u?.bv || 0),
      0,
    );

    user.leftPV = leftDocs.reduce(
      (s: number, u: { pv?: number } | undefined) => s + (u?.pv || 0),
      0,
    );
    user.rightPV = rightDocs.reduce(
      (s: number, u: { pv?: number } | undefined) => s + (u?.pv || 0),
      0,
    );
  }
...
function calculateTeamTotals(
  ...
  for (const [userId, user] of userMap) {
    ...
    user.totalLeftBV = leftIds.reduce(
      (sum, id) => sum + (userMap.get(id)?.bv || 0),
      0,
    );
    user.totalRightBV = rightIds.reduce(
      (sum, id) => sum + (userMap.get(id)?.bv || 0),
      0,
    );
    user.totalLeftPV = leftIds.reduce(
      (sum, id) => sum + (userMap.get(id)?.pv || 0),
      0,
    );
    user.totalRightPV = rightIds.reduce(
      (sum, id) => sum + (userMap.get(id)?.pv || 0),
      0,
    );
  }
...
function countActiveInSide(
  ...
  return ids.reduce((count, id) => {
    ...
    if (excludeAdmin && u.status_notes && /admin/i.test(u.status_notes))
      return count;
    return count + 1;
  }, 0);
}
