...
export async function POST(_request: Request) {
  try {
    ...
    const taggedDaily = (dailyPayouts as any[]).map((p) => ({
      ...p,
      _source: "daily" as const,
    }));
    const taggedWeekly = (weeklyPayouts as any[]).map((p) => ({
      ...p,
      _source: "weekly" as const,
    }));
    ...
    const addToGroup = (
      ...
      g.payout_records.push({
        ...
        account_holder_name:
          wallet?.account_holder_name || payout.account_holder_name || "",
        contact: wallet?.contact || payout.contact || "",
        mail: wallet?.mail || payout.mail || "",
        rank: payout.rank || "",
        wallet_id: wallet?.wallet_id || "",
        ...
        for (const rec of group.payout_records) {
          ...
          const releasedAmount = scoreBased
            ? group.withdraw_total > 0
              ? Math.round(
                  (rec.withdraw_amount / group.withdraw_total) * bal * 100,
                ) / 100
              : 0
            : rec.withdraw_amount;
    ...
    await Promise.all([
      scoreOps.length > 0 ? Score.bulkWrite(scoreOps) : Promise.resolve(),
