...
export async function POST(request: Request) {
  try {
    ...
    // ── USER: ALSO CREATE CHANGE REQUEST ───────────────────────────────
    if (role === "user") {
      ...
      // ── ADMIN: release payouts immediately ───────────────────────────
    ...
    return NextResponse.json(
      {
        ...
        message:
          role === "user"
            ? "Wallet created and request sent for admin approval."
  ...
  try {
    ...
    if (role === "user") {
      ...
      const changeRequest = await WalletChangeRequest.create({
        ...
        requested_role: role,
        old_values: wallet ? wallet.toObject() : {},
