...
export function getCurrentWindow() {
  ...
  if (istHours < 12) {
    ...
    end = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 5, 59, 59, 999)); // 11:59 AM IST = 5:59 UTC
  } else {
    // 12:00 PM - 11:59 PM IST
    start = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 6, 0, 0)); // 12:00 PM IST = 6:00 UTC
    end = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 17, 59, 59, 999)); // 11:59 PM IST = 17:59 UTC
  }
    ...
    for (const u of teamsAndHistories) {
      ...
      if (match) {
        ...
        await History.updateMany(
          { _id: { $in: [...u.left_histories, ...u.right_histories].map(h => h._id) } },
          { $set: { ischecked: true } }
        );

        console.log(`[Matching Bonus] Payout created for user: ${u.user_id}`);
      }
    }
